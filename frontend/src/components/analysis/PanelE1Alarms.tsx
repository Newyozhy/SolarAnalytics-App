...
// ─── Level mappings ──────────────────────────────────────────
const LEVEL_CONFIG: Record<number, { color: string; bg: string; icon: React.ElementType; label: string }> = {
  1: { color: 'text-red-500',   bg: 'bg-red-500/10',   icon: AlertOctagon, label: 'Critical' },
  2: { color: 'text-orange-500',bg: 'bg-orange-500/10',icon: AlertTriangle,label: 'Major' },
  3: { color: 'text-yellow-500',bg: 'bg-yellow-500/10',icon: AlertTriangle,label: 'Minor' },
  4: { color: 'text-blue-500',  bg: 'bg-blue-500/10',  icon: Info,          label: 'Warning' },
};

function AlarmRow({ alarm }: { alarm: AlarmRecord }) {
  const cfg = LEVEL_CONFIG[alarm.level] ?? LEVEL_CONFIG[4];
  const Icon = cfg.icon;
              ...
              {[1,2,3,4].map(l => (
                <button
                  ...
                  className={cn(
                    ...
                    levels.includes(l) ? LEVEL_CONFIG[l].bg + ' ' + LEVEL_CONFIG[l].color : "text-muted-foreground hover:bg-muted/50"
                  )}
                >
